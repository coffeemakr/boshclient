...
class BOSHClient(object):
    ...
    def wrap_stanza_body(self, stanza, more_body=''):
        ...
        if not stanza == '':
            return (
                "<body rid='%s' sid='%s' %s xmlns='http://jabber.org/protocol/httpbind'>" +
                "%s</body>"
            ) % (self.rid, self.sid, more_body, stanza)
        else:
            return "<body rid='%s' sid='%s' %s xmlns='http://jabber.org/protocol/httpbind' />" % (
                self.rid,
                self.sid,
                more_body
            )
    ...
    def register(self, **kwargs):
        ...
        xml_stanza = self.wrap_stanza_body(
    ...
    def request_bosh_session(self):
        ...
        xml_stanza = (
            "<body rid='%s' xmlns='http://jabber.org/protocol/httpbind' to='%s' xml:lang='en' " +
            "wait='60' hold='1' window='5' content='text/xml; charset=utf-8' ver='1.6' " +
            "xmpp:version='1.0' xmlns:xmpp='urn:xmpp:xbosh'/>"
        ) % (self.rid, self.jid.host)
        data = self.send_request(xml_stanza)
        ...
        auth_list = []
        try:
            mechanisms = stream_features.getElementsByTagNameNS(
    ...
    def xmpp_disco(self):
        ...
        xml_stanza = self.wrap_stanza_body(
            ...
            ) % (self.jid.jid_with_resource, self.jid.host)
        )
        data = self.send_request(xml_stanza)
        return data
    ...
    def xmpp_disco_node(self, node_name):
        ...
        xml_stanza = self.wrap_stanza_body(
            (
                "<iq type='get' from='%s' to='%s' id='info2'>" +
                "<query xmlns='http://jabber.org/protocol/disco#info' " +
                "node='http://jabber.org/protocol/%s'/></iq>"
            ) % (self.jid.jid_with_resource, self.jid.host, node_name)
        )
        data = self.send_request(xml_stanza)
        return data
    ...
    def authenticate_xmpp(self):
        ...
        if 'DIGEST-MD5' in self.server_auth:
            ...
            xml_stanza = (
                "<body rid='%s' xmlns='http://jabber.org/protocol/httpbind' sid='%s'>" +
            ...
            while continue_challenges:
                hash = minidom.parseString(
                    data
                ).documentElement.getElementsByTagName('challenge')[0].firstChild.data
            ...
            xml_stanza = (
                "<body rid='%s' xmlns='http://jabber.org/protocol/httpbind' sid='%s'>" +
                "<auth id='sasl2' xmlns='urn:ietf:params:xml:ns:xmpp-sasl' mechanism='PLAIN'>" +
                "%s</auth>" +
                "</body>"
            ) % (self.rid, self.sid, challenge)
            data = self.send_request(unicode(xml_stanza))
        ...
        xml_stanza = self.wrap_stanza_body(
            '',
            (
                "to='%s' xml:lang='en' xmpp:restart='true' " +
        ...
        xml_stanza = self.wrap_stanza_body(
            ...
            "<resource>%s</resource></bind></iq>" % self.resource
        )
        data = self.send_request(xml_stanza)
    ...
    def add_user(self, username, password):
        ...
        xml_stanza = self.wrap_stanza_body(
            (
                ...
                "node='http://jabber.org/protocol/admin#add-user'/></iq>"
            ) % (self.jid.jid_with_resource, id, self.jid.host)
        )
        data = self.send_request(xml_stanza)
        return data
...
class AdHocCommand(object):
    ...
    def __init__(self, jid, **kwargs):
        ...
        >>> my_command = AdHocCommand(
                'me@debian',
                id='get-registred-users-num-1',
                to='debian',
                type='set'
            )
        >>> my_command.set_command(
                xmlns=NS_COMMANDS,
                action='execute',
                node='http://jabber.org/protocol/admin#get-registered-users-num'
            ).string()
        "<iq from='me@debian' to='debian' type='set' id='get-registred-users-num-1'  xml:lang='en'>
            <command
                action='execute'
                node='http://jabber.org/protocol/admin#get-registered-users-num'
